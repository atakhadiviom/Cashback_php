...
<form method="post" action="<?= e(url('/wallet/reduce')) ?>" class="row g-3">
    ...
    <div class="col-md-4"><label class="form-label">مبلغ کسر</label><input class="form-control ltr" name="amount" inputmode="numeric" data-money required><?php if (!empty($errors['amount'])): ?><div class="form-text-error"><?= e($errors['amount']) ?></div><?php endif; ?></div>
    <div class="col-md-4"><label class="form-label">مبلغ خرید مرتبط (برای سقف درصد)</label><input class="form-control ltr" name="related_purchase_amount" inputmode="numeric" data-money placeholder="اختیاری"></div>
